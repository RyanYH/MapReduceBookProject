...
public class BookController {
	...
	public String movie_list(Model model) {
		// "/movie/list.jsp"
		List<BookVO> list = bgr.bookAllData();
...
//		model.addAttribute("bList", bList);
	...
	/*	if (feel == null) {
		...
		List<BookVO> mlist = new ArrayList<BookVO>();
		for (String v : flist) {
			BookVO dd = mgr.movieDetail(v);
			mlist.add(dd);
		}
...
/*	@RequestMapping("movie/total.do")
	public String movie_total(Model model) {
		List<BookVO> list = mgr.movieAllData();
		String value="";
		for(BookVO d:list)
		{
			value+="{ type:'column' "
